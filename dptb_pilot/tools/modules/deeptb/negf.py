...
def run_negf(
        ...
        config_file_name: str = "your_config_file_name",
        work_path: str = "."
) -> BandResult:
    """
    使用基准模型预测结构能带。
    ...
    参数:
        model_file_name: 使用的DeePTB模型。
        config_file_name: 输入的NEGF配置文件
    ...
    assert model_file_name != "your_model_file_name", "模型必须输入"
    assert config_file_name != "your_config_file_name", "negf的配置文件必须输入"

    # Resolve absolute paths
    work_dir = Path(work_path).absolute()
    config_path = work_dir / config_file_name
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        shutil.copy(config_path, temp_path / config_file_name)

        # Run dptb command in temp dir
        cmd = ['dptb', 'run', 'band', '-i', basemodel, '-stu', config_file_name, '-o', 'band_running']
